...
const listingSchema = new mongoose.Schema({
  ...
  address: {
    street: { type: String, required: [true, 'Rue est requise'], trim: true },
    city: { type: String, required: [true, 'Ville est requise'], trim: true },
    state: { type: String, trim: true },
    country: { type: String, required: [true, 'Pays est requis'], trim: true },
  ...
  pricing: {
    basePrice: { type: Number, required: [true, 'Prix de base est requis'], min: [1, 'Le prix doit être supérieur à 0'] },
    currency: { type: String, required: [true, 'Devise est requise'], enum: ['EUR', 'TND'], default: 'EUR' },
    cleaningFee: { type: Number, min: [0, 'Les frais de nettoyage ne peuvent pas être négatifs'], default: 0 },
    serviceFee: { type: Number, min: [0, 'Les frais de service ne peuvent pas être négatifs'], default: 0 },
    seasonalPricing: [{
  ...
  images: [{
    url: { type: String, required: [true, 'URL de l\'image est requise'] },
    caption: { type: String, maxlength: [200, 'La légende ne peut pas dépasser 200 caractères'] },
    isPrimary: { type: Boolean, default: false }
  }],
  availability: {
    minStay: { type: Number, min: [1, 'Séjour minimum doit être au moins 1 nuit'], default: 1 },
  ...
  houseRules: {
    ...
    additionalRules: [{ type: String, maxlength: [200, 'Chaque règle ne peut pas dépasser 200 caractères'] }]
  },
  // Dates bloquées manuellement par l'hôte (réservations externes, maintenance, etc.)
  externalBlocks: {
    type: [{
      startDate: { type: Date, required: true },
      endDate: { type: Date, required: true },
      reason: { type: String, default: 'Indisponible', maxlength: 200 },
  ...
  ratings: {
    ...
    count: { type: Number, min: [0, 'Nombre d\'avis ne peut pas être négatif'], default: 0 }
  }
}, {
  timestamps: true,
...
listingSchema.pre('save', function (next) {
  if (this.images && this.images.length > 0) {
    ...
    if (primaryImages.length === 0) {
      ...
      this.images.forEach((img, index) => { img.isPrimary = index === 0; });
    }
  }
  next();
});
